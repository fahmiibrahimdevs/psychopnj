<div>
        ...
        <div class="section-body">
                ...
                <div class="tw-tracking-wider tw-text-[#34395e] tw-ml-6 tw-mt-6 tw-mb-5 lg:tw-mb-1 tw-text-base tw-font-semibold tw-flex">
                    ...
                    <div class="ml-auto mr-3">
                        @if ((int) $seluruh_total_seharusnya == $seluruh_total_ditampilkan)
                            <span class="tw-bg-green-50 tw-rounded-md tw-text-sm tw-px-3 tw-py-1.5 tw-text-green-600 tw-tracking-wider">
                                <i class="fas fa-badge-check"></i>
                                BANK SOAL SIAP DIGUNAKAN
                            </span>
                        @else
                            <span class="tw-bg-red-50 tw-rounded-md tw-text-sm tw-px-3 tw-py-1.5 tw-text-red-600 tw-tracking-wider">
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-right tw-font-semibold tw-text-gray-800">{{ $seluruh_total_seharusnya }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right tw-font-semibold tw-text-gray-800">
                                            @if ((int) $seluruh_total_seharusnya == $seluruh_total_ditampilkan)
                                                <span class="tw-bg-green-50 tw-text-sm tw-rounded-md tw-px-3 tw-py-1.5 tw-text-green-600">SELESAI</span>
                                            @else
                                                <span class="tw-bg-red-50 tw-text-sm tw-rounded-md tw-px-3 tw-py-1.5 tw-text-red-600">BELUM SELESAI</span>
                                            @endif
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Status Bank Soal</td>
                                        <td class="text-right tw-flex tw-items-center tw-justify-end tw-gap-2">
                                            <span class="tw-text-sm tw-font-semibold {{ $this->bankSoal->status == "1" ? "tw-text-green-600" : "tw-text-gray-500" }}">
                                                {{ $this->bankSoal->status == "1" ? "Aktif" : "Tidak Aktif" }}
                                            </span>
                                            <label class="switch tw-mb-0">
                                                <input type="checkbox" wire:click="updateStatus('{{ $this->bankSoal->status == "1" ? "0" : "1" }}')" {{ $this->bankSoal->status == "1" ? "checked" : "" }} />
            ...
            <div class="card">
                <div class="card-body px-4">
                    <div>
                        <button wire:click="jenis('1')" class="btn {{ $selectedJenis === "1" ? "btn-primary" : "btn-dropdown" }} tw-tracking-wider">Pilihan Ganda</button>
                        <button wire:click="jenis('2')" class="btn {{ $selectedJenis === "2" ? "btn-primary" : "btn-dropdown" }} tw-tracking-wider">Pilihan Ganda Kompleks</button>
                        <button wire:click="jenis('3')" class="btn {{ $selectedJenis === "3" ? "btn-primary" : "btn-dropdown" }} tw-tracking-wider">Menjodohkan</button>
                        <button wire:click="jenis('4')" class="btn {{ $selectedJenis === "4" ? "btn-primary" : "btn-dropdown" }} tw-tracking-wider">Isian Kompleks</button>
                        <button wire:click="jenis('5')" class="btn {{ $selectedJenis === "5" ? "btn-primary" : "btn-dropdown" }} tw-tracking-wider">Essai/Uraian</button>
                    </div>
                    <div>
                        <table class="mt-3">
                            <thead>
                ...
                <div class="card-body px-4">
                    <div class="tw-flex">
                        <p class="tw-font-semibold tw-text-gray-700 tw-tracking-wider">
                            {{
                                $selectedJenis == "1"
                                    ? "A. Soal Pilihan Ganda (Opsi: $bankSoal->opsi Opsi) "
                                    : ($selectedJenis == "2"
                            ...
                            <div class="card-body px-4">
                                                ...
                                                @if (! empty($row[$option]))
                                                    ...
                                                    @elseif ($selectedJenis == "2")
                                                        @foreach (json_decode($row[$option], true) as $key => $opsiKompleks)
                                                            <li class="tw-flex tw-ml-3">{{ $key }}. &nbsp;{!! $opsiKompleks !!}</li>
                                                        @endforeach
                                                    @endif
                                                @endif
                                            @endforeach
                                ...
                                @if ($selectedJenis == "3")
                                    <div class="tampil-jodohkan-container mt-2">
                                        <div class="tampil-jodohkan no-scrollbar" id="tampil-jodohkan-{{ $row["id"] }}"></div>
                                    </div>
                                @elseif ($selectedJenis == "5")
                                    <div class="tw-bg-gray-50 tw-p-4 tw-border-2 tw-border-dashed tw-border-gray-300 tw-rounded-sm">
    ...
    <div class="modal fade" wire:ignore.self id="formDataModal" aria-labelledby="formDataModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <form>
                    <div class="modal-body">
                        ...
                        @if ($selectedJenis == "1")
                            @if (in_array($bankSoal->opsi, ["3", "4", "5"]))
                                @foreach (range("A", chr(64 + $bankSoal->opsi)) as $option)
                                    <div class="form-group">
                        ...
                        @elseif ($selectedJenis == "2")
                            <button wire:click.prevent="addOpsiKompleks" class="btn btn-primary mb-4">
                            ...
                            @foreach ($opsi_kompleks as $key => $value)
                                <div class="form-group">
                                    <div class="mb-2">
                                        <button wire:click.prevent="removeOpsiKompleks('{{ $key }}')" class="btn btn-sm btn-danger mr-1">
                                            <i class="fa fa-trash tw-text-base"></i>
                                        </button>
                                        <span class="tw-text-gray-700 tw-text-[13px] tw-font-semibold">Opsi {{ $key }}</span>
                                        <button wire:click.prevent="toggleCorrectOpsiKompleks('{{ $key }}')" class="float-right mt-2">
                                            <label class="switch">
                                                <input type="checkbox" {{ in_array($key, $opsi_benar_kompleks) ? "checked" : "" }} />
                                                <span class="slider round"></span>
                                            </label>
                                        </button>
                                    </div>
                                    <div wire:ignore>
                                        <textarea data-key="{{ $key }}" id="opsi_kompleks.{{ $key }}" class="opsi_kompleks form-control mt-2"></textarea>
                                    </div>
                                </div>
                            @endforeach
        ...
        <div id="loading-image-summernote">
        ...
        @push("general-css")
            ...
            <style>
                ...
                #loading-image-summernote .spinner-border {
                    width: 3rem;
                    height: 3rem;
                }
            </style>
